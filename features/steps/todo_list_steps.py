...
@given('the to-do list containstasks: "{tasks}"')
def given_tasks(context, tasks):
    task_list = tasks.split(", ")
    context.manager = ToDoListManager()
    for task in task_list:
        context.manager.add_task(task)
...
@given('the to-do listcontains tasks:"{tasks}"')
def given_tasks2(context, tasks):
    task_list = tasks.split(", ")
    context.manager = ToDoListManager()
    for task in task_list:
        context.manager.add_task(task)
...
def step_then_check_empty(context):
    assert len(context.manager.list_tasks()) == 0, "The to-do list is not empty."


@given('theto-do list contains tasks:"{tasks}"')
def step_given_tasks2(context, tasks):
    task_list = tasks.split(", ")
    context.manager = ToDoListManager()
    for task in task_list:
        context.manager.add_task(task)
...
@then('the to-do list should contain"{task}"')
def step_then_check_task2(context, task):
    tasks = [t['task'] for t in context.manager.list_tasks()]
